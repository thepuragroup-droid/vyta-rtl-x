...
export interface ProductsClientProps {
  /** Active products, already sorted, resolved server-side by app/products/page.tsx. */
...
export default function ProductsClient({ initialProducts }: ProductsClientProps) {
  ...
  // The grid is server-rendered from a page cached for 5 minutes, so the stock
  // counts in it can be slightly stale — and stock decides both the
  // out-of-stock badge and the sort. Re-read the catalog once on mount and swap
  // it in. A failure leaves the server-rendered list in place, which is the
  // right fallback: slightly stale beats empty.
  useEffect(() => {
    let cancelled = false;
    supabase
      .from('products')
      .select('stock_quantity, price, vial_price, id')
      .eq('active', true)
      .then(({ data, error }) => {
        if (cancelled || error || !data) return;
        const fresh = new Map(data.map((row) => [row.id, row]));
        setProducts((current) =>
          current
            .map((product) => ({ ...product, ...(fresh.get(product.id) ?? {}) }))
            .sort((a, b) => {
  ...
  // Restore scroll position. The grid is server-rendered, so it is already
  // laid out on the first paint and there is nothing to wait for.
  useEffect(() => {
